...
{
    let located = false
    let paddleCenter = player2.y + (player.height / 2)
    if (paddleCenter > target - 20 && paddleCenter < target + 20)
        located = true
    if (located == false)
    {
...
function AIX()
{
    if (player2.canHit)
    {
        let distSide = canvas.width - ball.x
        let timeSide = distSide/ball.velocityX
        if (ball.velocityY < 0)
        {
            let distTop = ball.y
            let timeTop = Math.abs(distTop/ball.velocityY)
            if (timeSide < timeTop)
            {
                ballWillBe = timeSide * ball.velocityY + ball.y
                moveTo(ballWillBe)
            }
            else if (timeSide - timeTop < 0.1)
            {
                ballWillBe = timeSide * ball.velocityY + ball.y
                moveTo(ballWillBe)
            }
        ...
        {
            let distBottom = canvas.height - ball.y
            let timeBottom = distBottom/ball.velocityY
            if (timeSide < timeBottom)
            {
                ballWillBe = timeSide * ball.velocityY + ball.y
                moveTo(ballWillBe)
            }
            else if (timeSide - timeBottom < 0.1)
            {
                ballWillBe = timeSide * ball.velocityY + ball.y
                moveTo(ballWillBe)
            }
...
function AI()
{
    if (player2.canHit)
    {
        ...
        if (ball.velocityY < 0)
        {
            ...
            {
                ballWillHitX = timeTop * ball.velocityX + ball.x
                ballWillHitY = 0
                newDistSide = canvas.width - ballWillHitX
                newDistTop = ballWillHitY

                newTimeSide = newDistSide / ball.velocityX
                newTimeTop = Math.abs(newDistTop/ball.velocityY)

                if (newTimeSide < newTimeTop)
                {
                    ballWillBe = newTimeSide * ball.velocityY + ballWillHitY
                    moveTo(ballWillBe)
                }
            }
        }
        else
        {
            ...
            {
                ballWillHitX = timeBottom * ball.velocityX + ball.x
                ballWillHitY = canvas.height
                newDistSide = canvas.width - ballWillHitX
                newDistBottom = canvas.height - ball.y

                newTimeSide = newDistSide / ball.velocityX
                newTimeBottom = Math.abs(newDistBottom/ball.velocityY)

                if (newTimeSide < newTimeBottom)
                {
                    ballWillBe = newTimeSide * ball.velocityY + ballWillHitY
                    moveTo(ballWillBe)
                }
